...
public class BinarySearch {
    public static void main(String[] args) {
        ...
        List<Character> charList = new ArrayList<>(Arrays.asList('a', 'b', 'c', 'd', 'e'));
    ...
    public static <T extends Comparable<T>> int binarySearch(List<T> list, T target) {
        ...
        if(target instanceof Integer || target instanceof Double || target instanceof String || target instanceof Character
                || target instanceof Float || target instanceof Long || target instanceof Short || target instanceof Byte
    ...
    private static <T extends Comparable<T>> int implementBinarySearch(List<T> list, T target) {
        int left = 0, right = list.size() - 1, mid;
        while (left <= right) {
            mid = (left + right) >> 1;
            if (list.get(mid).equals(target)) return mid;
            else if (list.get(mid).compareTo(target) < 0) left = mid + 1;
            else right = mid - 1;
        }
        return -1;
    }
